...
class myImage:
    ...
    def __init__(self, width, height, myScale, minGeo, maxGeo, midGeo, xw650, yw450, myNight=True):
        ...
        response = requests.get(mapRequest) # в try-except нужно это...!!! если fail, то просто чёрное.. и на диск сохранять !!! 100кб * 500 камер * 500 городов = 2гб500мб, сойдёт думаю.. !!!
...
def probkShowHTML(request):
    ...
    def getJsOnCached(myUrl, mySec = 10, myJsOnStorage = '/va'):
        ...
        myJsOnText='{nothing}'
        response = requests.get(myUrl) # это в try.. и здесь ещё, видимо, timeout чёто указать, чтоль... 
    ...
    mymess+=" - timeSpent : "+str(time.time()-myTimeStart)
